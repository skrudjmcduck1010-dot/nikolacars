...
class DonorPartDisplayPresenter
{
    ...
    public function categoryForDonor(DonorCar $donorCar, mixed $catalogItem = null): ?PartCatalogCategory
    {
        if (! $catalogItem instanceof PartCatalogItem) {
            return null;
        }

        $occurrenceCategory = $catalogItem->occurrences
            ->pluck('category')
    ...
    {
        $catalogPath = $this->categoryPath($this->categoryForDonor($donorCar, $product->sourcePartCatalogItem), 'preferred', false, true, true);
    ...
    {
        $catalogPath = $this->categoryPath($this->categoryForDonor($donorCar, $sale->partCatalogItem), 'preferred', false, true, true);
    ...
    {
        $isGeneratedDonorProduct = $product->generated_at !== null
            && preg_match('/^DON\d+-/i', (string) $product->sku) === 1;
        $isAutoGenerated = ((bool) $product->is_auto_generated || $isGeneratedDonorProduct)
