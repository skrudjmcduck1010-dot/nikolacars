...
class NikolaCarsInventoryService
{
    ...
    public function itemGroups(Collection $items, array $usdRate, callable $displayItemName): Collection
    {
        ...
        return $this->buildItemGroups($groups, $usdRate, $displayItemName);
    }

    public function itemGroupsForIndividualItems(Collection $items, array $usdRate, callable $displayItemName): Collection
    {
        $groups = $items->mapWithKeys(fn (PartCatalogItem $item): array => [
            'item:'.$item->getKey() => collect([$item]),
        ]);

        return $this->buildItemGroups($groups, $usdRate, $displayItemName)
            ->keyBy(fn (array $group): int => (int) $group['item']->getKey());
    }

    protected function buildItemGroups(Collection $groups, array $usdRate, callable $displayItemName): Collection
    {
        $items = $groups
        ...
        $productPhotoBuckets = $this->productPhotoBucketsForItems($items);
        $officialPartImageUrls = $this->officialPartImageUrlsForItems($items);
        $productDonorColors = $this->productDonorColorsForItems($items);
        $productAutoGeneratedFlags = $this->productAutoGeneratedFlagsForItems($items);
        $reservationDetailsByGroup = $this->reservationDetailsByGroup($groups);

        return $groups
            ->map(function (Collection $group, string $groupKey) use ($displayItemName, $officialPartImageUrls, $productAutoGeneratedFlags, $productDonorColors, $productPhotoBuckets, $productPrices, $productQuantities, $reservationDetailsByGroup, $usdRate): array {
                /** @var PartCatalogItem $first */
                $first = $group->first();
                $reservationDetails = $reservationDetailsByGroup[$groupKey] ?? [
                    'quantity' => 0.0,
                ...
                $productPhotoUrls = $this->productImageUrlsForGroup($group, $productPhotoBuckets);
                $teslaPhotoUrls = $this->teslaImageUrlsForGroup($group, $productPhotoBuckets, $officialPartImageUrls);
                $imageUrls = $productPhotoUrls->isNotEmpty() ? $productPhotoUrls : $teslaPhotoUrls;
                ...
                return [
                    ...
                    'gallery_image_urls' => $this->mergeImageUrlCollections($productPhotoUrls, $teslaPhotoUrls),
                    'product_image_urls' => $productPhotoUrls,
                    'tesla_image_urls' => $teslaPhotoUrls,
                    'names' => $group
                    ...
                    'has_auto_generated_product' => $this->groupHasAutoGeneratedProduct($group, $productAutoGeneratedFlags),
    ...
    protected function productAutoGeneratedFlagsForItems(Collection $items): Collection
    {
        $productIds = $this->productIdsForItems($items);
        ...
        return Product::query()
            ->whereIn('id', $productIds->all())
            ->get(['id', 'is_auto_generated', 'generated_at'])
            ->mapWithKeys(fn (Product $product): array => [
                $product->id => (bool) $product->is_auto_generated || $product->generated_at !== null,
            ]);
    }

    protected function groupHasAutoGeneratedProduct(Collection $group, Collection $productAutoGeneratedFlags): bool
    {
        return $group->contains(function (PartCatalogItem $item) use ($productAutoGeneratedFlags): bool {
            $productId = $this->productIdFromItem($item);

            return $productId > 0 && (bool) $productAutoGeneratedFlags->get($productId, false);
        });
    }
    ...
    protected function productPhotoBucketsForItems(Collection $items): Collection
    {
        $productIds = $this->productIdsForItems($items);
        ...
        return Product::query()
            ...
            ->mapWithKeys(fn (Product $product): array => [
                $product->id => [
                    'product' => $this->inventoryProductPhotoUrls($product),
                    'tesla' => $this->inventoryTeslaPhotoUrls($product),
                ],
            ]);
    }

    protected function inventoryProductPhotoUrls(Product $product): Collection
    {
        return $this->normalizeImageUrls(ProductPhotoNormalizer::productPhotos($product)
            ->reject(fn (string $path): bool => $this->isTeslaPartImageUrl($path)));
    }

    protected function inventoryTeslaPhotoUrls(Product $product): Collection
    {
        return $this->normalizeImageUrls(ProductPhotoNormalizer::productPhotos($product)
            ->filter(fn (string $path): bool => $this->isTeslaPartImageUrl($path)));
    }
    ...
    public function imageUrlsForItem(PartCatalogItem $item): Collection
    {
        return $this->imageUrlsForGroup(
            collect([$item]),
            $this->productPhotoBucketsForItems(collect([$item])),
            $this->officialPartImageUrlsForItems(collect([$item])),
        );
    }

    protected function imageUrlsForGroup(Collection $group, Collection $productPhotoBuckets, ?Collection $officialPartImageUrls = null): Collection
    {
        $officialPartImageUrls ??= collect();
        $productPhotoUrls = $this->productImageUrlsForGroup($group, $productPhotoBuckets);
        $teslaPhotoUrls = $this->teslaImageUrlsForGroup($group, $productPhotoBuckets, $officialPartImageUrls);

        return $productPhotoUrls->isNotEmpty() ? $productPhotoUrls : $teslaPhotoUrls;
    }

    protected function productImageUrlsForGroup(Collection $group, Collection $productPhotoBuckets): Collection
    {
        return $this->normalizeImageUrls($group
            ->flatMap(function (PartCatalogItem $item) use ($productPhotoBuckets): Collection {
                $productId = $this->productIdFromItem($item);
                $bucket = $productPhotoBuckets->get($productId);

                if ($productId > 0 && $productPhotoBuckets->has($productId)) {
                    return $bucket['product'] ?? collect();
                }

                return collect((array) data_get($item->raw_attributes, 'image_urls', []))
                    ->reject(fn (mixed $url): bool => $this->isTeslaPartImageUrl((string) $url));
            }));
    }

    protected function teslaImageUrlsForGroup(Collection $group, Collection $productPhotoBuckets, Collection $officialPartImageUrls): Collection
    {
        return $this->normalizeImageUrls($group
            ->flatMap(function (PartCatalogItem $item) use ($officialPartImageUrls, $productPhotoBuckets): Collection {
                $productId = $this->productIdFromItem($item);
                $partNumber = $this->normalizePartNumber((string) $item->part_number);
                $bucket = $productPhotoBuckets->get($productId);
                $productTeslaUrls = $productId > 0 && $productPhotoBuckets->has($productId)
                    ? ($bucket['tesla'] ?? collect())
                    : collect();
                $rawTeslaUrls = collect((array) data_get($item->raw_attributes, 'image_urls', []))
                    ->filter(fn (mixed $url): bool => $this->isTeslaPartImageUrl((string) $url));

                return $productTeslaUrls
                    ->merge($officialPartImageUrls->get($partNumber, collect()))
                    ->merge($rawTeslaUrls);
            }));
    }

    protected function normalizeImageUrls(iterable $urls): Collection
    {
        return collect($urls)
    ...
    protected function mergeImageUrlCollections(Collection ...$collections): Collection
    {
        return $collections === []
            ? collect()
            : $this->normalizeImageUrls(collect($collections)->flatMap(fn (Collection $urls): Collection => $urls));
    }

    protected function isTeslaPartImageUrl(string $url): bool
    {
        $raw = Str::lower(rawurldecode(trim($url)));
        $displayUrl = Str::lower(rawurldecode(trim((string) (PublicStorageUrl::url($url) ?? $url))));

        return Str::contains($raw, 'tesla-official/part-images/')
            || Str::contains($displayUrl, 'tesla-official/part-images/');
    }
        ...
        $candidatesByPrefix = PartCatalogItem::query()
            ...
            ->where(function ($query) use ($prefixes): void {
                foreach ($prefixes as $prefix) {
                    $query->orWhere('part_number', 'like', $prefix.'%');
                }
            })
    ...
    public function activeItemsQuery()
    {
        ...
        return PartCatalogItem::query()
            ->with('category:id,parent_id,source,name,name_en,name_ru,name_ua,code,source_url,depth,model_label')
