...
{
    private const REASON = 'auto: unchecked donor dimension pattern with Tesla.com price under 2 USD';
        ...
        DB::table('products')
            ...
            ->chunkById(500, function (Collection $products) use (&$partNumbers): void {
                ...
                foreach ($products as $product) {
                    ...
                    if (! $this->hasDimensionPattern($officialItem)) {
                        continue;
                    }
    ...
    protected function hasDimensionPattern(?object $officialItem): bool
    {
        $name = trim((string) ($officialItem?->name_en ?: $officialItem?->name ?: ''));

        return $name !== '' && preg_match('/\d+(?:\.\d+)?\s*x\s*\d+(?:\.\d+)?/i', $name) === 1;
    }
