...
{
    private const REASON = 'auto: unchecked donor nut with Tesla.com price under 2 USD';
        ...
        DB::table('products')
            ...
            ->chunkById(500, function (Collection $products) use (&$partNumbers): void {
                ...
                foreach ($products as $product) {
                    ...
                    if (! $this->isNutName($officialItem)) {
                        continue;
                    }
    ...
    protected function isNutName(?object $officialItem): bool
    {
        $name = trim((string) ($officialItem?->name_en ?: $officialItem?->name ?: ''));

        return $name !== '' && stripos($name, 'nut') !== false;
    }
