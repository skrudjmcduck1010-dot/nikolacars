...
{
    private const REASON = 'auto: unchecked donor clip with Tesla.com price under 2 USD';
        ...
        DB::table('products')
            ...
            ->chunkById(500, function (Collection $products) use (&$partNumbers): void {
                ...
                foreach ($products as $product) {
                    ...
                    if (! $this->isClipName($officialItem)) {
                        continue;
                    }
    ...
    protected function isClipName(?object $officialItem): bool
    {
        $name = trim((string) ($officialItem?->name_en ?: $officialItem?->name ?: ''));

        return $name !== '' && stripos($name, 'clip') !== false;
    }
